...
public class ProfessorService implements ProfessorServiceContract {
    ...
    public void create(ProfessorDTO professor) {
        ...
        if (cursoList.size() != professor.getCursoList().size()) {
            throw new RegraNegocioException("Um ou mais cursos não foram encontrados dentro da lista.");
        }
    ...
    public void update(Long id, UpdateListLongDTO cursoList) {
        Professor professorEncontrado = professorRepository.findById(id).orElseThrow(
                () -> new RegraNegocioException("Não foi possível encontrar o professor pelo ID fornecido."));
